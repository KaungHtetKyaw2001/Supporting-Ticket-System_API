...
public function show($guid)
{
    ...
    public function update(Request $request, string $guid)
    {
        ...
        $ticketData = Validator::make($request->all(),[
            ...
            'content' => 'required|string|max:255',
            'message' => 'required|string|max:255',
            // 'attachment_file' => 'sometimes|required|string|max:2048', // Add validation rule for attachment file
        ]);
    ...
    public function userAccountUpdate(Request $request, string $guid)
    {
        $timestamp = Carbon::now()->setTimezone('Asia/Yangon')->format('Y-m-d\TH:i:s.uO');
        ...
        if(!$user)
        {
            return response()->json([
                'success' => false,
                'message' => 'The user you find does not exist',
                'timestamp' => $timestamp], 404);
        }
        else
        {
    ...
    public function index()
    {
        ...
        if($ticket->count()>0)
        {
           return response()->json([
                'success' => true,
                'data' => [$ticket],
                'message' => 'All Ticket data Retrieved',
                'timestamp' => $timestamp
           ]);
        }
        else
